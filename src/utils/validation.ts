...
export function validateComponentName(name: string): void {
  ...
  if (!/^[A-Z][a-zA-Z0-9]*$/.test(name)) {
    throw new ValidationError('Component name must start with uppercase letter and contain only alphanumeric characters');
  }
}
...
export function validateLayout(layout: string): string {
  const validLayouts = ['vertical', 'horizontal', 'grid', 'single-column', 'two-column', 'inline'];
...
export function validateColor(color: string): string {
  ...
  if (!hexRegex.test(color) && !rgbRegex.test(color) && !rgbaRegex.test(color)) {
    throw new ValidationError(`Invalid color format: ${color}. Use hex (#fff), rgb(), or rgba() format`);
  }
